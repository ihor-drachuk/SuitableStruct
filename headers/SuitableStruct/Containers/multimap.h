...
struct SuitableStruct::ContainerItemType<std::multimap<Args...>>
{
    using C = std::multimap<Args...>;
    using type = typename std::pair<typename C::key_type, typename C::mapped_type>;
};

template<typename... Args>
struct SuitableStruct::ContainerInserter<std::multimap<Args...>>
{
    using C = std::multimap<Args...>;
...
template<typename... Args> struct SuitableStruct::IsContainer<std::multimap<Args...>> : public std::true_type { };
